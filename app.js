        ...
        switch (type) {
            ...
            case 'boiler_el': {
                ...
                return `<span style="${styles}"><span style="${head}">Электрический котёл STOUT ${isStatus ? 'STATUS' : 'PLUS'}</span>` +
                       `<b>Зачем:</b> Дополнительный (резервный) или основной источник тепла в системе отопления. Котлы серии ${isStatus ? 'STATUS' : 'PLUS'} представляют собой мини-котельную со встроенным насосом и расширительным баком, обеспечивая бесшумную работу и высокий уровень автоматизации.<br><br>` +
                       `<b>Формула подбора:</b> Q_требуемая = Q_теплопотери × 1.1 (коэффициент запаса 10% по СП 60.13330.2020). При каскаде (мощность > 27 кВт): N_котлов = 2 (поскольку максимальная паспортная мощность одного прибора в линейке составляет 27 кВт).<br><br>` +
                       `<b>Подставленные значения:</b><br>` +
                       `• Расчетные теплопотери: ${targetPwr.toFixed(1)} кВт.<br>` +
