...
--
-- Что здесь НАМЕРЕННО не сводится в одно поле «источник». Правило такое:
-- оплата важнее промокода, потому что pro_expires_at заполняется ТОЛЬКО при
-- выборе «Оплата» в карточке, а distributor_id ставится независимо от тарифа
-- («не зависит от тарифа» написано в самой форме) — это адрес для копии
-- запроса счёта.
--
-- В админке на момент написания этого файла порядок был обратный, и
-- оплативший монтажник с указанным поставщиком считался промокодным
-- (исправлено в app.js тем же днём). Строка всё равно хранит ОБА исходных
-- признака — paid и distributor_id — плюс производный source: если правило
-- когда-нибудь поменяется снова, историю можно пересчитать, ничего не
-- потеряв. Сам SQL при этом не переписывался: он уже выполнен, и файл
-- остаётся записью того, что реально ушло в базу.
--
